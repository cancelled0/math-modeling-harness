...
description: 统一调度数学建模竞赛任务。适用于完整赛题、目标模糊或跨阶段请求；识别当前阶段，调用 workflow-orchestrator 读取状态，并把一个主要动作路由给正确的专业 Skill，而不替代专业分析。
---
...
2. 调用 `workflow-orchestrator` 检查当前 Qx、rigor profile、阶段门、阻塞项和允许动作。
...
每次调度只报告：当前 profile、当前 Qx/阶段门、证据或阻塞项、一个主要下一动作、需要人工确认的理由（若有）。随后执行已获授权且不需要人工判断的动作。
