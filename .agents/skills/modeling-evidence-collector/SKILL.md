...
- 至少有 `planning/problem_contract.json`、Qx 目标和缺失证据清单。
...
2. 按配置的 `source_scan`、`abstract_review` 或 `fulltext_analysis` 深度检索；记录来源层级，网页转载只作线索。
...
按项目 AGENTS.md 的证据充分性标准，将未覆盖项写入 evidence brief。
...
- submission 的每个正式 Qx 都有 evidence brief，或有“无需外部证据”的可审计理由；lean 只在任务风险需要时采集。
